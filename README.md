...
Updated `markdown-it` dependency to latest to avoid dependency issue in monorepo. The older version of `markdown-it` was using an older version of the `entities` package which seem to be used across many NPM modules and had breaking changes.
...
```npm
yarn add @jonasgroendahl/react-native-markdown-display
```

#### NPM

```npm
npm install -S @jonasgroendahl/react-native-markdown-display
```
...
import Markdown from '@jonasgroendahl/react-native-markdown-display';
